...
contract Ballot {
    ...
    function participants() external view returns (uint) {
    ...
    function open(uint bounty, address[] calldata whitelist_) external payable {
    ...
    function getId(address voter) external view returns (uint id) {
    ...
    function closeVote() external returns (Vote winningVote) {
      ...
      winningVote = votes[Vote.YES] > votes[Vote.NO] ? Vote.YES : votes[Vote.YES] < votes[Vote.NO] ? Vote.NO : Vote.ABS;
      // reset votes mapping
      votes[Vote.YES] = 0;
      votes[Vote.NO] = 0;
      votes[Vote.ABS] = 0;
    ...
    function vote(uint voterId, Vote t) external {
